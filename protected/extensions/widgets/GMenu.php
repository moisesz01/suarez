...
class GMenu extends CWidget
{
	...
	protected function renderMenuRecursive($items)
	{
		...
		{
			$item['itemOptions']=array('class'=>'dropdown','id'=>'menu1');
	...
	protected function renderMenuItem($item)
	{
		if(isset($item['url']))
		{
			...
			{
			if($item['url']!=='')
				$item['linkOptions']['href']=CHtml::normalizeUrl($item['url']);
				return CHtml::tag('a',isset($item['linkOptions']) ? $item['linkOptions'] : array(), $label."<b class=\"caret\"></b>");
			}else
